		...
		unsigned int *num_channel_groups)
{
	...
	* customizable, so this is intentionally a little more dynamic than
	* strictly necessary for the "stock" models, to make it more likely
	* to automatically support end-user upgrades of the various ranges.
	*
	* The BB3 in particular supports various different modular power supply
	* frontends that offer different voltage/current limits and different
	* numbers of independent channels, such as three PSU modules that have
	* two channels each for a total of six controllable channels.
	*
	* This currently supports both the original EEZ PSU design (H24005, when
	* in its stock build configuration) and the successor EEZ BB3 design.
	*/

	struct sr_scpi_dev_inst *scpi = (struct sr_scpi_dev_inst *)sdi->conn;
	ret = sr_scpi_get_int(scpi, ":SYST:CHAN:COUN?", &channel_count);
	if (ret != SR_OK) {
	...
	if (channel_count > ARRAY_SIZE(eez_psu_channel_names)) {
		/*
		 * No known EEZ PSU specifies more than six channels at the time of
		 * writing, so it would be weird to get here but we'll allow it to
		 * be robust.
		 */
		sr_warn("Only first %d of %d channels of EEZ PSU are supported by the current driver", channel_count, ARRAY_SIZE(eez_psu_channel_names));
		channel_count = ARRAY_SIZE(eez_psu_channel_names);
	}

	sr_dbg("EEZ PSU has channel count %d", channel_count);

	*channels = g_malloc0(sizeof(struct channel_spec) * channel_count);
	*channel_groups = g_malloc0(sizeof(struct channel_group_spec) * channel_count);
	for (i = 0; i < channel_count; i++) {
		channel_name = eez_psu_channel_names[i];

		// Select the channel to prepare for our various "get" calls below.
		ret = sr_scpi_send(scpi, ":INST:NSEL %s", channel_name);
		if (ret != SR_OK) {
			sr_err("Failed to select %s to retrieve its limits", channel_name);
			return ret;
		}
		...
		ret = sr_scpi_get_double(scpi, ":SYST:CHAN:INFO:CURR?", &limit_val);
		if (ret != SR_OK) {
			sr_err("Failed to read the current limit for %s", channel_name);
			return ret;
		}
		...
		ret = sr_scpi_get_double(scpi, ":SYST:CHAN:INFO:VOLT?", &limit_val);
		if (ret != SR_OK) {
			sr_err("Failed to read the voltage limit for %s", channel_name);
			return ret;
		}
		...
		ret = sr_scpi_get_double(scpi, ":SYST:CHAN:INFO:POW?", &limit_val);
		if (ret != SR_OK) {
			sr_err("Failed to read the power limit for %s", channel_name);
			return ret;
		}
